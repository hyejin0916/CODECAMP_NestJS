...
export class ProductsService {
  constructor(
    ...
    private readonly productsSaleslocationsService: ProductsSaleslocaionsService,
  ) {}
  ...
  async create({
    ...
    console.log('product1', productSaleslocation);
    console.log('product2', product);

    const result = await this.productsSaleslocationsService.create({
      ...productSaleslocation,
    }); // 서비스를 타고 가야 하는 이유: Repository에 직접 접근하면 검증 로직을 통일시킬 수 없음

    const result2 = this.productsRepository.save({
      ...product,
      productSaleslocation: result, // result 통째로 넣기(id 만 넣은 경우 id만 조회 가능)
    });
